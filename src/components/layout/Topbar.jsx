...
function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between gap-4 border-b border-white/70 bg-white/80 px-4 py-4 backdrop-blur-xl sm:px-6">
      <div className="flex items-center gap-3">
        <Button className="md:hidden" onClick={openSidebar} size="sm" variant="ghost">
          <Menu className="h-5 w-5" />
        </Button>
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-brand-600">{t("layout.appName")}</p>
          <h2 className="text-xl font-extrabold text-ink-900">{pageTitle}</h2>
        </div>
      </div>

      <div className="flex items-center gap-3">
        <LanguageSwitcher compact />
        <div className="hidden items-center gap-3 rounded-2xl border border-ink-200 bg-white px-3 py-2 sm:flex">
